        ...
        for i, char in enumerate(s):
            if char in chars_req:
                ...
                if p1 == -1:
                    p1 = i
                p2 = i
                # `s` has an answer - add it and move to 2nd  Phase
        ...
        while p2 - p1 > min_dist:
            if all(v < 1 for v in chars_req.values()):
                ...
                if p1_char in chars_req:
                    chars_req[p1_char] += 1
                    # Keep removing same char, if answer still valid (lookahead)
                    while p1 < last_ix and s[p1+1] == p1_char and chars_req[p1_char] < 0:
                        p1 += 1
                        chars_req[p1_char] += 1
                        answer = s[p1:p2+1]
                p1 += 1
